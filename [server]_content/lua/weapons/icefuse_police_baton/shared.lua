...
File Path:   addons/[server]_content/lua/weapons/icefuse_police_baton/shared.lua
...
function SWEP:AttackPlayer(ply)
  ...
  if ply:GetDataVar("adminmode") then DarkRP.notify(ply, 1, 3, "That mother fucker tried to hit you with a baton!") return end --Icefuse Networks Development (Psyche): Staff/admin baton prevention ;)
...
function SWEP:AttackEnt(ent,dmg)
        ...
        if FPP and FPP.plyCanTouchEnt(self:GetOwner(), ent, "EntityDamage") then
         ...
         ent:TakeDamage(1000 - dmg, self:GetOwner(), self);
		end
end
/* 
---------------------------------------------------------------------------------------------------------------------------------------------
